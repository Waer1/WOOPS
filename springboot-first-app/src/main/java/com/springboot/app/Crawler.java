    ...
    public boolean Crawel(String url) {
        ...
        try {
            ...
            if (connection.response().statusCode() == 200) {
                ...
                {
                    System.out.println("Its NOt html page...");
                    return false;
                }
                ...
                for (Element link : hyperLinkes.select("a[href~=^[^#]+$]")) {
                    this.links.add(link.absUrl("href"));
                }
            } else {
                System.out.printf("Failed to get HTML from %s..\n", url);
                return false;
            }

        } catch (IOException ioe) {
            System.out.println("Failed make http request");
